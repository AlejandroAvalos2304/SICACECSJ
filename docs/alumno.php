...
<?php include('../docs/AgregarAModal.php'); ?>
    ...
    <script type="text/javascript" class="init">
        function Alertabaja(id, nombre, apellido) {
            ...
            alertify.confirm("<center>ATENCI&Oacute;N!</center>", "<center><img src='../images/warning.png' width='200' height='200'></center>" + "<center><h2>¿Desea Dar De Baja A?</h2></center>  <center><h3> " + nombre + " " + apellido + "  </h3></center>",
        ...
        function alertaExito() {
            alertify.success("<p>Se dio de baja exitosamente</p>" + "<img src='../images/bien.png' width='80' height='80'>").set({
                transition: 'flipx'
            });
        }

        function alertaError() {
            alertify.error("<p>No se puedo dar de baja</p>" + "<img src='../images/error.png' width='80' height='80'>").dismissOthers();
                    ...
                    <div class="card">
                        <div class="header">
                            ...
                            <ul class="header-dropdown m-r--5">
                                <button type="button" class="btn btn-primary waves-effect waves-float" style="float: right;" data-toggle="modal" data-target="#AgregarModal"><img src="../images/iconos/registrar.svg" alt="x" />
                                    Registrar Alumno
                                </button>
                            </ul>
                        </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                        ...
                                        <tbody>
                                            ...
                                            $query_s = pg_query($conexion, "SELECT alumnos.*, inscripcion.* from alumnos INNER JOIN inscripcion ON inscripcion.idalum = alumnos.idalum where alumnos.estadoa=1");
                                            while ($fila = pg_fetch_array($query_s)) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php echo $fila["apellidoa"]; ?></td>
                                                    <td><?php echo $fila["grado"]; ?></td>
                                                    <td><?php echo $fila["seccion"]; ?></td>

                                                    <td>
                                                        <button type="button" name="ver" value="Ver" class="btn btn-info waves-effect waves-float ver_data" data-toggle="modal" data-target="#ModalVer_<?php echo $rid = $fila["idalum"]; ?>"><img src="../images/iconos/baseline-chrome_reader_mode-24px.svg" /></butoon>
                                                    </td>
                                                    <?php include '../docs/VerAModal.php'; ?>
                                                    <td>
                                                        <button type="button" name="edit" value="Edit" class="btn btn-warning waves-effect waves-float edit_data" data-toggle="modal" data-target="#ModalEdi_<?php echo $rid = $fila[0]; ?>"><img src="../images/iconos/baseline-edit-24px.svg" /></button>
                                                    </td>
                                                    <td>
                                                        <button type="button" name="baja" value="Baja" class="btn btn-danger waves-effect waves-float baja_data" onClick="Alertabaja('<?php echo $fila["idalum"]; ?>','<?php echo $fila["nombrea"]; ?>','<?php echo $fila["apellidoa"]; ?>')"><img src="../images/iconos/thumb-down.svg" /></button>
                                                    </td>
                                                    <?php include '../docs/EditarAModal.php'; ?>
                                                </tr>
                                            <?php
                                            }
                                            ?>
...
if (isset($_REQUEST["bandera"])) {
    ...
    if ($bandera == "baja") {
        ...
        $result2 = pg_query($conexion, "UPDATE alumnos SET estadoa='$estadoa' where idalum='$baccion'");

        if (!$result2) {
            pg_query("rollback");
            echo "<script language='javascript'>";
            echo "alertaError();";
            echo "setTimeout ('r()', 3000);";
            echo "</script>";
        } else {
            pg_query("commit");
